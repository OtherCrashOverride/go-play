...
void audio_init(int rate)
{
    ...
    snd.bufsize = (rate / 60);
    printf("audio_init: snd.bufsize=%d\n", snd.bufsize);
...
//    memset(snd.psg_buffer[1], 0, snd.bufsize * 2);
    ...
    SN76489_Init(0, MASTER_CLOCK, rate);
...
void system_save_state(void *fd)
{
    ...
    //abort();
    fwrite(&PSG, sizeof(PSG), 1, fd);

    // SRAM
    fwrite(sms.sram, SRAM_SIZE, 1, fd);
}
...
void system_load_state(void *fd)
{
    ...
    //abort();
    fread(&PSG, sizeof(PSG), 1, fd);

    // SRAM
    fread(sms.sram, SRAM_SIZE, 1, fd);
